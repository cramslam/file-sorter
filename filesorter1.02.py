...
import os, shutil, customtkinter, json, sys
...
root.title("Super Cool File Sorter - v1.02")
...
file3_path = os.path.join(base_path, 'assets/file3.png')
img1_path = os.path.join(base_path, 'assets/scfs1.1.png')
icon_path_ico = os.path.join(base_path, 'assets/icon4.ico')

#set window and taskbar icon using the .ico file
root.iconbitmap(icon_path_ico)

#load the .ico image and use it for both the taskbar and window icon
icon_image = Image.open(icon_path_ico)
icon_photo = ImageTk.PhotoImage(icon_image)

#set the taskbar and window icon using iconphoto
...
#logo
img1 = customtkinter.CTkImage(Image.open(img1_path), size=(190,115))
...
entry1.grid(row= 1, column= 0, padx=(12,5), pady= (6,6))

#file button
img2 = customtkinter.CTkImage(Image.open(file3_path), size=(20,20))
file_button = customtkinter.CTkButton(master=frame, image= img2, text="", width=1, command=lambda: filepath(entry1))
file_button.grid(row= 1, column= 1, padx=(0,12), pady= (6,6), sticky= "W")
...
textbox.insert("insert", "- Welcome to the Super Cool File Sorter!\n- How to use: Choose or enter the file path that you want to sort\n- Select which file type(s) you want to sort\n- Click sort and watch the program work")
textbox.configure(state = DISABLED)



#credit label
label4 = customtkinter.CTkLabel(master=root, text="Version 1.02 - Created by Liam Shaw", font=("Arial", 10, "bold"))
